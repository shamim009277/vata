...
const showCollectPaymentModal = ref(false);
const payments = ref([]);
...
const openCollectPaymentModal = (invoice) => {
    ...
    showCollectPaymentModal.value = true;
};
...
const printPaymentReceipt = (p) => {
    ...
    const makeCopy = (label) => `
        <div class="receipt">
            ...
            <table class="table">
                ...
                <tbody>
                    ${((selectedInvoice.value?.invoice_details || []).map(d => `
                        <tr>
                            <td>${d.item?.name || '-'}</td>
                            <td class="num">${Number(d.rate).toFixed(2)}</td>
                            <td class="num">${Number(d.quantity).toFixed(2)}</td>
                            <td class="num">${Number(d.amount).toFixed(2)}</td>
                        </tr>
                    `).join('')) || `
            ...
            <div class="summary">
                ...
                <div><span>গাড়ি ভাড়া</span><span class="num">${Number(selectedInvoice.value?.rant || 0).toFixed(2)}</span></div>
                <div><span>ছাড়</span><span class="num">${Number(selectedInvoice.value?.discount || 0).toFixed(2)}</span></div>
                <div class="bold"><span>সর্বমোট</span><span class="num">${Number(selectedInvoice.value?.total_amount || p.total_amount).toFixed(2)}</span></div>
    ...
    <head>
        ...
        <style>
            ...
            .contact { text-align: center; color: #555; margin-bottom: 14px; }
    ...
    <body>
        <div class="watermark">${props.business_store?.store_name || ''}</div>
        <div class="grid">
...
const submitCollectPayment = () => {
    spinBtn.value = true;
    axios.post(route('invoices.collectPayment', selectedInvoice.value.id), paymentForm.value)
        .then(response => {
            spinBtn.value = false;
            showCollectPaymentModal.value = false;
            toast.success(response.data.message || 'পেমেন্ট সফলভাবে সংগ্রহ করা হয়েছে!');
            router.reload({ preserveState: true });
        })
        .catch(error => {
            spinBtn.value = false;
            toast.error(error.response?.data?.message || 'পেমেন্ট সংগ্রহ করতে ব্যর্থ হয়েছে।');
        });
};
...
<template>
    ...
    <AppLayout1>
                        ...
                        <div class="card-title d-flex justify-content-between align-items-center flex-wrap gap-2 mb-0">
                            ...
                            <h6 class="mb-0 text-primary d-flex align-items-center">
                                <a href="javascript:;" class="me-2">
                                    <i class="fadeIn animated bx bx-list-ul"></i> সব বাকি লিস্ট
                                </a>
                            </h6>
                                            ...
                                            <td>
                                                <div class="dropdown dropstart">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        ...
                                                        <li v-if="invoice.due_amount > 0">
                                                            <a class="dropdown-item" href="#" @click="openCollectPaymentModal(invoice)">
                        ...
                        <form @submit.prevent="invoiceDelivary">
                            ...
                            <div class="modal-footer d-flex justify-content-between">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="smsSwitchAll" v-model="delivaryForm.send_sms">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="modal-footer">
                                <button class="btn btn-secondary btn-sm" @click="showPaymentsModal = false"><i class="bx bx-x me-1"></i> Close</button>
                            </div>
                    </div>
                </div>
            </div>
        ...
        <transition name="modal">
            <div class="modal-backdrop" v-if="showCollectPaymentModal"></div>
        </transition>
        <transition name="slide-fade">
            <div class="modal d-block" v-if="showCollectPaymentModal">
                <div class="modal-dialog modal-md">
                    <div class="modal-content">
                        <div class="modal-header" style="border-top: 2px solid #004882;">
                            <h6 class="modal-title">
                                <i class="bx bx-wallet me-2"></i>
                                পেমেন্ট সংগ্রহ করুন (চালান: {{ selectedInvoice?.invoice_no }})
                            </h6>
                            <button type="button" class="btn-close" @click="showCollectPaymentModal = false"></button>
                        </div>
                        <form @submit.prevent="submitCollectPayment">
                            <div class="modal-body">
                                <div class="row g-2">
                                    <div class="col-12 mb-2">
                                        <div class="customer-danger text-center">
                                            <h4 class="text-danger">বাকিঃ {{ Number(selectedInvoice?.due_amount || 0).toFixed(2) }}</h4>
                                            <p class="text-danger"><strong>পরিশোধের তারিখ:</strong> {{ formatDate(selectedInvoice?.next_payment_date) }}</p>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <label class="form-label">তারিখ</label>
                                        <Input type="date" v-model="paymentForm.payment_date" class="form-control form-control-sm" required/>
                                    </div>
                                    <div class="col-sm-6">
                                        <label class="form-label">পেমেন্ট মেথড</label>
                                        <select v-model="paymentForm.payment_method" class="form-control form-control-sm" required>
                                            <option value="cash">ক্যাশ</option>
                                            <option value="mobile_banking">মোবাইল ব্যাংকিং</option>
                                            <option value="check">চেক</option>
                                        </select>
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <Input type="number" step="any" min="0" v-model="paymentForm.paid_amount" class="form-control form-control-sm" required/>
                                    </div>
                                    <div class="col-sm-6" v-if="paymentForm.payment_method === 'mobile_banking'">
                                        <label class="form-label">একাউন্ট নম্বর</label>
                                        <Input v-model="paymentForm.account_number" class="form-control form-control-sm" placeholder="একাউন্ট নম্বর" />
                                    </div>
                                    <div class="col-sm-6" v-if="paymentForm.payment_method === 'check'">
                                        <label class="form-label">চেক নম্বর</label>
                                    ...
                                    <div class="col-12">
                                        <label class="form-label">নোট</label>
                                        <Input v-model="paymentForm.note" class="form-control form-control-sm" placeholder="নোট" />
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button class="btn btn-secondary btn-sm" @click="showCollectPaymentModal = false"><i class="bx bx-x me-1"></i> বাতিল</button>
                                <button type="submit" class="btn btn-primary btn-sm">
                                    <i v-if="spinBtn" class="bx bx-loader bx-spin"></i>
