...
const editingRawMaterial = ref(false);
...
const createRawMarerials = () => {
    showModal.value = true;
    editingRawMaterial.value = false;
    form.reset();
}

const openEdit = (rawMaterial) => {
    showModal.value = true;
    editingRawMaterial.value = rawMaterial;

    form.name = rawMaterial.name;
    form.type = rawMaterial.type;
    form.unit_id = rawMaterial.unit_id;
    form.cost_per_unit = rawMaterial.cost_per_unit;
    form.stock_alert_quantity = rawMaterial.stock_alert_quantity;
    form.is_active = rawMaterial.is_active;
}

const updateStatus = (rawMaterial) => {
    router.put(`/raw-materials/${rawMaterial.id}/status`, {
        is_active: rawMaterial.is_active ? 1 : 0
    }, {
        preserveScroll: true,
        onSuccess: () => {
...
const submit = () => {
    if (editingRawMaterial.value) {
        form.put(`/raw-materials/${editingRawMaterial.value.id}`, {
            preserveScroll: true,
            onSuccess: () => {
                form.reset();
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="example" class="table table-striped table-bordered dataTable" style="width: 100%;" role="grid" aria-describedby="example_info">
                                            ...
                                            <tbody>
                                                <tr v-for="(rawMaterial, index) in rawMaterials.data" :key="rawMaterial.id">
                                                    <td>{{ index + 1 }}</td>
                                                    <td>{{ rawMaterial.name }}</td>
                                                    <td>{{ rawMaterial.type }}</td>
                                                    <td>{{ rawMaterial.unit.name }}</td>
                                                    <td>{{ rawMaterial.cost_per_unit }}</td>
                                                    ...
                                                    <td>
                                                        <a @click="openEdit(rawMaterial)" class="text-primary" style="cursor: pointer;"><i class="fadeIn animated bx bx-edit hover:opacity-90" style="font-size: larger;"></i></a>
                                                        <a @click.prevent="confirmDelete(rawMaterial.id)" class="text-danger" style="cursor: pointer;"><i class="fadeIn animated bx bx-trash hover:opacity-90" style="font-size: larger;"></i></a>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                ...
                                <div class="row">
                                    <div class="col-sm-12 col-md-5">
                                        <div class="dataTables_info" role="status" aria-live="polite">
                                            Showing {{ rawMaterials.from }} to {{ rawMaterials.to }} of {{ rawMaterials.total }} entries
                                        </div>
                                    </div>
